...
function offlinePage() {
  return `<!DOCTYPE html>
  <html>
    <head><title>Playwright Runner Offline</title></head>
    <body style="font-family:sans-serif;text-align:center;padding:50px">
      <h1>\u{1F534} Playwright Runner is offline</h1>
...
export default {
  async fetch(request, env) {
    ...
    if (path === "/status") {
      ...
      try {
        const check = await fetch(`${tunnelUrl}/health`, {
